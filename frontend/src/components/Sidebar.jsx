...
const Sidebar = () => {
  ...
  return (
    <aside
      className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col border-r border-base-300 bg-base-100 lg:flex"
    ...
    >
      <div className="border-b border-base-300 px-5 py-4">
        ...
        >
          ...
          <div>
            ...
            <span className="block text-[11px] uppercase tracking-[0.16em] opacity-55">
              Stay connected
            </span>
          </div>
        </Link>
        ...
        {navigation.map(({ to, label, icon, chatBadge, notificationBadge }) => {
          ...
          return (
            <Link
              ...
              className={`flex min-h-11 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary ${
                active
            ...
            >
              ...
              <span>{label}</span>
              {chatBadge && <ChatUnreadBadge className="ml-auto" />}
              {notificationBadge && <NotificationCountBadge className="ml-auto" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="border-t border-base-300 p-4">
        <Link
          to="/profile"
          className="flex items-center gap-3 rounded-xl p-2 transition-colors hover:bg-base-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
        >
          <ProfileAvatar
