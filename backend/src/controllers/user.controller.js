...
async function attachRelationshipState(users, currentUser) {
  ...
  return plainUsers.map((user) => ({
    ...
    relationshipStatus: getRelationshipStatus({
      ...
      pendingRequest: requestByTargetId.get(
        String(user._id)
      ),
    }),
  }));
...
export async function updateMyProfile(req, res) {
  ...
  if (
    ...
    try {
      ...
      logger.warn(
        "Unable to update Stream user during profile update",
        error
      );
    }
  }
...
export async function getPublicProfile(
  ...
  const pendingRequest = idsEqual(
    id,
    req.user.id
  )
    ? null
    : await getPendingRequestBetween(
        req.user.id,
        id
      );

  const relationshipStatus = idsEqual(
    id,
    req.user.id
  )
 ...
 * and users who are already friends.
 */
function buildDiscoveryFilter(req) {
  return {
    _id: {
      $ne: req.user.id,
...
/*
 * Search:
 * exclude only the currently logged-in user.
 *
 * Existing friends are intentionally included so
 * global people search can find them as well.
 */
...
async function getDiscoveryPage(
  req,
  filter,
  sort
) {
  ...
  const friendRequest =
    await FriendRequest.findOneAndUpdate(
      ...
      },
      {
        $set: {
          status:
            FRIEND_REQUEST_STATUS.CANCELLED,
        },
      },
      { new: true }
    );
...
export async function removeFriend(req, res) {
  ...
  if (idsEqual(friendId, req.user.id)) {
    return failure(
      res,
      400,
  ...
  const currentHasFriend = hasFriend(
    currentUser.friends,
    friendId
  );

  const friendHasCurrent = hasFriend(
    friend.friends,
    req.user.id
  );
