...
export const PROFILE_PUBLIC_FIELDS =
  "_id fullName bio profilePic uploadedProfilePic avatarProfilePic profileImageMode nativeLanguage learningLanguage location isOnboarded createdAt";
...
const limits = {
  ...
  learningLanguage: 60,
  profilePic: 500,
  uploadedProfilePic: 500,
  avatarProfilePic: 500,
};
...
export function buildProfileUpdatePayload(body = {}) {
  const payload = {};

  for (const field of PROFILE_EDITABLE_FIELDS) {
    if (field in body) {
      payload[field] = normalizeProfileText(body[field]);
    }
  }
...
export function validateProfileUpdate(payload) {
  if (!Object.keys(payload).length) {
    return "Provide at least one editable profile field";
  }
  ...
  if (
    ...
    !["photo", "avatar"].includes(payload.profileImageMode)
  ) {
    return "Profile image mode must be photo or avatar";
  }
...
export function validatePaginationQuery(
  query = {},
  maxLimit = MAX_DISCOVERY_LIMIT
) {
  for (const key of ["page", "limit"]) {
    if (query[key] === undefined) continue;

    if (!/^\d+$/.test(String(query[key]))) {
      return `${key} must be a positive integer`;
    }

    if (Number(query[key]) < 1) {
      return `${key} must be a positive integer`;
    }
  }
...
export function parsePagination(query = {}) {
  ...
  const limit =
    Number.isFinite(rawLimit) && rawLimit > 0
      ? Math.min(rawLimit, MAX_DISCOVERY_LIMIT)
